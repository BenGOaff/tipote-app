...
```bash
cmp lib/quiz/champsPersonnalises.ts ../tiquiz/lib/quiz/champsPersonnalises.ts
```
...
| | |
|---|---|
| strippers maison routés | 11 chez Tipote, 9 chez Tiquiz |
| champs riches rendus bruts en JSX | 6 chez Tipote, 4 chez Tiquiz |
| dont **`SurveyTrends`** | **l'écran exact où son client a vu l'entité**, dans les DEUX dépôts |
| dont `lib/leadAnswers.ts` | les réponses d'un lead, rendues dans l'admin, les exports et les emails |
...
### ET CE QUE LE PORTAGE A RÉVÉLÉ ICI

Le chantier du 15 septembre (le consentement qui affichait
`confidentialité&nbsp;` en clair) était décrit dans l'AGENTS.md de
Tiquiz comme porté chez Tipote. **Mesuré le 16 septembre : il ne
l'était pas.** Ni `lib/quiz/consentement.ts`, ni `decodeHtmlEntities`,
ni le test n'existaient ici, et `ConsentText` rendait encore `{raw}`.

Les trois sont arrivés avec ce chantier, et la note de Tiquiz est
corrigée en place. **Quand une section annonce un portage, la dernière
étape n'est pas de l'écrire, c'est d'aller CHERCHER le fichier chez le
jumeau** : `ls ../tipote-app/lib/quiz/consentement.ts`, dix secondes.
