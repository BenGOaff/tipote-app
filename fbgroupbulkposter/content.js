...
async function openComposerAndInsert(text, images, video_id, scheduled, background) {
  const ocaiId = Date.now() + "-" + Math.floor(Math.random() * 10000);
  console.debug("[ocai] openComposerAndInsert called", { id: ocaiId, textPreview: (text || "").substr(0, 120), imagesCount: (images || []).length, video_id, scheduled, background });
  if (text) {
    ...
    let done = await injectIntoComposer(text, imgs, video_id, scheduled, background);
    if (!done) {
...
async function logTelemetry(
  ...
  try {
    ...
    let u = await chrome.storage.sync.get(["user"]);
    if (chrome.runtime.lastError) {
      console.error("Storage error (telemetry user):", chrome.runtime.lastError);
      data["email"] = "unknown";
    } else {
      data["email"] = u?.user?.email || "unknown";
    }
...
async function injectIntoComposer(
  ...
  if (!inputElement && (images && images.length || video_ids)) {
    ...
    if (scheduled) {
      ...
      await fetchSnapshotViaPort?.(video_ids).catch(() => { });
    } else {
  ...
  try {
    ...
    // Optional: store a little marker so other parts of extension know we're done
    if (chrome?.storage?.local) {
      chrome.storage.local.set({ operationDone: "done" }, () => {
        if (chrome.runtime.lastError) {
          console.error("Storage error:", chrome.runtime.lastError);
        }
      });
    ...
    await waitForPostCompletion?.(500, video_ids, clickedPost);
...
async function waitForPostCompletion(interval = 500, snapShotId, clickedPost) {
  return new Promise((resolve) => {
    const check = () => {
