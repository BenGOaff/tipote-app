...
type QuizPromptParams = {
  ...
  niche?: string;
  mission?: string;
  /** Voix de marque condensée (tonalité écrite + offres + puces promesses +
   *  persona douleurs/désirs), assemblée par brandLoader. Fait écrire l'IA DANS
   *  la voix de l'user plutôt qu'en voix neutre. */
  brandVoice?: string;
  /** Exemples de structures d'accroches/promesses (ressources copywriting). */
  copyHint?: string;
  locale?: string;
  addressForm?: "tu" | "vous";
  format?: "short" | "long";
...
// Output schema identical to buildQuizGenerationPrompt so downstream parser is agnostic.
...
RÔLE : On te fournit le contenu brut d'un quiz (copié d'un doc, d'un brouillon, d'un PDF). Ta mission : le transformer en JSON quiz exploitable par Tipote, SANS inventer de contenu qui ne serait pas dans le texte source.
...
${NATURAL_WRITING_BLOCK}
${brandVoice ? `\nVOIX DE LA MARQUE — écris DANS cette voix (inspire-t'en, ne recopie pas) :\n${brandVoice}\n` : ""}
