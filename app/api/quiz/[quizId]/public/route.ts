...
export async function GET(_req: NextRequest, context: RouteContext) {
  try {
    ...
    // Les colonnes RECENTES vivent a part, pour que le repli ci-dessous
    // les retire toutes d'un coup si la migration n'est pas passee.
    ...
    if (quizUserId) {
      ...
      // Masquer completement le footer : reserve aux plans payants (meme gate
      // que le footer perso). Un free ne peut jamais retirer la mention.
      quizPublic.hide_branding = footerAllowed && Boolean(quizPublic.hide_branding);
    }
    ...
    return NextResponse.json({
      ...
      quiz: {
        ...
        close_enabled: (quizPublic as any).close_enabled === true,
        close_action: ((quizPublic as any).close_action as string | null) ?? null,
        close_redirect_url: ((quizPublic as any).close_redirect_url as string | null) ?? null,
        close_message: ((quizPublic as any).close_message as string | null) ?? null,
        close_cta_text: ((quizPublic as any).close_cta_text as string | null) ?? null,
        close_cta_url: ((quizPublic as any).close_cta_url as string | null) ?? null,
      },
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const { data: quiz } = await admin
      .from("quizzes")
      .select("id, user_id, project_id, title, meta_pixel_id, mode, sio_capture_tag, sio_score_tags, scoring_axes, score_labels, locale")
      .eq("id", quizId)
      .eq("status", "active")
      .maybeSingle();
    ...
    // segmenter l'emailing selon le diagnostic.
    ...
    if ((quiz as { sio_score_tags?: boolean | null }).sio_score_tags === true && scoresSnapshot) {
      ...
      for (const axis of normalizeScoringAxes((quiz as { scoring_axes?: unknown }).scoring_axes)) {
        const sc = scoresSnapshot.axes?.[axis.id];
        if (!sc || sc.max - sc.min <= 0) continue;
        scoreTags.push(`${axisSlug(axis)}-${trancheSlug(scorePercent(sc))}`);
      }
    }
    if (resultId || surveyCaptureTag || surveyHasAnswers || scoreTags.length > 0) {
