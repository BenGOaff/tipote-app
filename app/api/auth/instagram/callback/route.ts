...
// Callback OAuth Instagram : echange le code, recupere les Pages Facebook,
// decouvre le compte Instagram Business/Creator lie a la Page,
// stocke le Page token chiffre (l'API IG utilise le Page token).
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // IMPORTANT : passer le redirect_uri Instagram (pas celui de Facebook)
    console.log("[Instagram callback] Exchanging code for token...");
    const shortLived = await exchangeCodeForToken(code, getInstagramRedirectUri());
    ...
    // Fallback granular_scopes si /me/accounts est vide
    if (pages.length === 0) {
