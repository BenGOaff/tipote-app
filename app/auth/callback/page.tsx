...
// Rôle : page de callback Supabase (invite / recovery / magic link).
    ...
    (async () => {
      try {
        ...
        if (code) {
          ...
          // Après exchange, session dispo côté browser
          const { data } = await supabase.auth.getSession();
          const session = data?.session;

          // Si on a un "type" en query (parfois), on l'honore.
          ...
          // Fallback : si pas de session, retour login
          if (!session) {
        ...
        if (access_token && refresh_token) {
          ...
          // Nettoie l'URL (évite de laisser les tokens dans l’historique)
          try {
      ...
      <div className="w-full max-w-md rounded-2xl bg-slate-900/80 border border-slate-800 p-6 shadow-lg">
        {status === "loading" ? (
          ...
          <>
            ...
            <p className="text-sm text-slate-400">
              {errorMsg || "Impossible de finaliser la connexion. Réessaie depuis l’email."}
            </p>
            <button
              type="button"
