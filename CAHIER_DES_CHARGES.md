...
Navigation : Vue d'ensemble, Promouvoir, Mes liens, Contenus, Revenus, Paiement, Essai gratuit, Support. Revenus et Paiement existaient sans figurer dans aucun menu depuis le passage à la sidebar : un affilié ne pouvait ni consulter ses commissions ni comprendre comment il est payé sans connaître l'URL.
...
- Promouvoir : les deux liens principaux (Atelier et Tiquiz) côte à côte en tête, puis les liens trackés éditables par l'affilié (libellé, description, destination), le paramètre `?ref=<code public>` étant ajouté automatiquement. Persistance dans `affiliates.promo_overrides`.
- Contenus : espace rangé en dossiers, sur le modèle de "Mes projets" côté Tiquiz. Racine "Contenu", puis un dossier par produit ("Promouvoir l'Atelier du Quiz", "Promouvoir Tiquiz"), puis cinq rayons identiques dans chacun : Emails de vente, Réseaux sociaux, Articles de blog, Logo et branding, Générer du contenu. Fil d'Ariane à chaque niveau. Le lien tracké injecté est celui du produit du dossier courant.
  - Emails : la campagne complète, avec les objets alternatifs A/B/C copiables séparément, la copie qui conserve le gras (Systeme.io) et la copie en texte brut. Le kit Atelier compte 15 emails, avec les parcours d'envoi conseillés (7 envois, 3 envois).
  - Réseaux : un post par carte, avec son visuel. Image seule à télécharger, ou carrousel qui défile sur place avec deux boutons dessous, le PDF prêt à publier et le zip de toutes les slides en PNG (route `/affiliate/api/assets/carousel`, zip écrit sans dépendance). Le lien tracké est proposé à part, à coller en premier commentaire, jamais dans le corps du post. Chaque post peut aussi déclencher le studio visuel IA.
  - Logo et branding : les fichiers officiels du produit (logo, version fond foncé, icône, jaquette, visuels de présentation) avec leurs règles d'usage.
  - Générer du contenu : atelier d'écriture assisté, cadré produit par produit. L'affilié décrit son audience, choisit un format (email, post, article, script vidéo court ou long), et récupère un texte modifiable, copiable, avec le visuel générable dans la foulée. Les faits produits, les tarifs, la garantie et les règles d'écriture vivent côté serveur (`lib/affiliate/generatorBrief.ts`) : le modèle n'invente ni chiffre, ni promesse, ni URL, refuse le hors sujet, et les tirets cadratins sont filtrés en sortie. 20 générations par heure et par affilié.
  - Les emails, posts et articles de la banque admin sont désormais rangés par produit (colonne `affiliate_contents.product`), avec un sélecteur Tiquiz / Atelier dans l'admin.
...
- Guide de lancement : 6 étapes (profil, lien copié, coordonnées de versement renseignées, essai activé, premier email envoyé, premier post publié), 3 détectées automatiquement et 3 auto-déclarées, affiché en haut de la vue d'ensemble avant les compteurs tant qu'il n'est pas terminé.
- Revenus : totaux par statut, historique des commissions, et simulateur de gains à deux produits. Le simulateur part des prix publics réels (Atelier du Quiz à 47 € en paiement unique, Tiquiz à 17 €/mois pour l'accès simple ou 29 €/mois pour le Plus, sélecteur de plan inclus) et des taux réels (70% et 40% sur les 12 premiers mois), montre le gain du mois et le cumul sur 12 mois en tenant compte du récurrent, et n'avance aucun taux de conversion moyen.
- Mes liens : un tableau, une ligne par lien nommé, avec ses propres clics, ses inscrits et ses commissions. Le meilleur canal en premier (trier par date mettrait son plus vieux lien en haut). Le lien du tableau passe par NOTRE redirecteur `/go/<code>/...` : un lien qui va droit sur la page de vente commissionne toujours, mais ses chiffres restent à zéro pour toujours. Le lien par défaut ne se supprime pas (il vit dans des vidéos déjà publiées). Les chiffres du bandeau sont la SOMME du tableau. Cartes sur mobile, pas un tableau à sept colonnes.
- Paiement : **le cycle de versement vit chez nous depuis le 25 août.** L'affilié choisit PayPal ou virement, saisit son adresse PayPal OU son IBAN, remplit son profil fiscal et accepte le mandat d'autofacturation ; il y retrouve aussi ses factures. Ce qui reste chez Systeme.io, et la page le dit : les commissions des ventes arrivées par leurs anciens tunnels.
