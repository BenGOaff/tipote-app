    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-1rem)] sm:w-[1200px] sm:max-w-[95vw] h-[90vh] flex flex-col p-0 gap-0">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1 bg-muted rounded-lg p-1">
              {(Object.keys(DEVICE_CONFIG) as Device[]).map((d) => {
                ...
                return (
                  <Button key={d} variant={device === d ? "secondary" : "ghost"} size="sm" className="h-8 px-3 gap-1.5" onClick={() => setDevice(d)}>
        ...
        <div className="flex-1 overflow-auto bg-muted/50 flex items-start justify-center p-6">
          <div className="bg-white rounded-lg shadow-xl overflow-hidden transition-all duration-300 border" style={{ width: cfg.width, maxWidth: "100%", minHeight: 600 }}>
