...
type QuizData = {
  ...
  brand_font: string | null; brand_color_primary: string | null; brand_color_background: string | null; brand_color_text: string | null;
...
export default function SurveyDetailClient({ quizId }: SurveyDetailClientProps) {
  ...
  // reglage editable, ET dans les dependances du memo : sans la
  // dependance, le memo ne se recalcule pas, donc ecrire un libelle ne
  // declenche AUCUN enregistrement, et rien ne le dit.
  // `SURVEY_SNAPSHOT_KEYS` le reclame, et le compilateur refuse un
  ...
  const handleExportCSV = () => {
    ...
    // fin de ligne (16 septembre 2026), depuis la MÊME liste pour l'en-tête
    // et la ligne.
    const champs = champsVisibles(customFields);
    const csv = [[t("csvEmail"), t("csvFirstName"), t("csvLastName"), t("csvResult"), t("csvDate"), ...champs.map((c) => c.label)].map(h => `"${String(h).replace(/"/g,'""')}"`).join(","), ...leads.map(l => [l.email, l.first_name ?? "", l.last_name ?? "", stripHtml(l.result_title ?? ""), l.created_at ? new Date(l.created_at).toLocaleDateString() : "", ...champs.map((c) => valeurChamp(l.custom_fields, c.id) ?? "")].map(c => `"${String(c).replace(/"/g,'""')}"`).join(","))].join("\n");
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3 text-sm">
              ...
              {leftTab === "design" && (<div className="space-y-5">
                ...
                <div className="space-y-3"><Label className="text-xs">{t("designColorsLabel")}</Label>
                  ...
                  {/* Couleur des autres textes. NULL tant que non choisie -> aucun override. */}
                      ...
                      <div className="max-w-2xl w-full space-y-8">
                        ...
                        {qType === "free_text" && (() => {
                          ...
                          return (
                            <div className="space-y-3">
                              <div className="relative">
                                <textarea readOnly rows={5} maxLength={maxLength} className="w-full rounded-xl border-2 px-4 py-3 text-base resize-none bg-muted/10" style={{ borderColor: `${pc}30` }} />
